...
def main():
    parser = argparse.ArgumentParser(
        description="Run C++ code coverage for scikit-kamila."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", default="coverage_cpp", help="Directory for output reports."
    )
    ...
    if is_windows and shutil.which("OpenCppCoverage"):
        ...
        if args.xml:
            occ_cmd.extend(
                ["--export_type", f"cobertura:{out_dir / 'coverage-cpp.xml'}"]
            )
        ...
        if shutil.which("gcovr"):
            ...
            print(
                "[-] 'gcovr' not found in PATH. Install via 'pip install gcovr' "
