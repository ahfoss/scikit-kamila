...
To create your package, you need to clone the ``scikit-kamila`` repository:

.. prompt:: bash $

  git clone https://github.com/ahfoss/scikit-kamila.git

Before to reinitialize your git repository, you need to make the following
changes. Replace all occurrences of ``kamila``, ``scikit-kamila``, or
``scikit-kamila`` with the name of you own project. You can find all the
occurrences using the following command:

.. prompt:: bash $

  git grep kamila
  git grep scikit-kamila
  git grep scikit-kamila
