...
def all_estimators(type_filter=None):
    """Get a list of all estimators from `kamila`.
    ...
    all_classes = []
    root = str(Path(__file__).parent.parent)  # kamila package
...
def all_displays():
    """Get a list of all displays from `kamila`.
...
def _is_checked_function(item):
    ...
    mod = item.__module__
    if not mod.startswith("kamila.") or mod.endswith("estimator_checks"):
        return False
...
def all_functions():
    """Get a list of all functions from `kamila`.
